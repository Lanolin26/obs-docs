# Введение в Iptables
![ Схема работы Iptables:](picture/2608017e6b7d35b36aa066de1f411f74.jpg " Схема работы Iptables:")
...
В этом примере команда `-N MY_CHAIN` создает новую цепочку с именем MY\_CHAIN. Затем добавляется правило, которое отбрасывает все пакеты от 192\.168\.0\.100\. Наконец, цепочка MY\_CHAIN добавляется в цепочку INPUT, что означает, что все входящие пакеты будут также проходить через цепочку MY\_CHAIN.
